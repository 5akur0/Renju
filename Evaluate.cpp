...
EVALUATION chessAi::evaluate(int board[15][15]) {
    ...
    for (i = 1; i < 17; ++i) {
        ...
        if (i == 1) eval.STAT[1] = count; // WIN
        else if (i == 2) eval.STAT[2] = count; // LOSE
        else if (i == 3) eval.STAT[3] = count; // FLEX4
        else if (i == 5) eval.STAT[5] = count; // BLOCK4
        else if (i == 7) eval.STAT[7] = count; // FLEX3
    }
    eval.result = R_DRAW;

    // 白赢
    if (eval.STAT[1] > 0) eval.result = R_WHITE;
    // 黑赢
    else if (eval.STAT[2] > 0) eval.result = R_BLACK;
